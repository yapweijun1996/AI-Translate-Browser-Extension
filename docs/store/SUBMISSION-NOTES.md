...
| Host permissions for `https://gpt.yapweijun1996.com/*`, `https://generativelanguage.googleapis.com/*`, `https://api.openai.com/*`, and `https://api.deepseek.com/*` | Allow the service worker to call the demo session/response endpoints and, only when the user explicitly selects a configured BYOK engine, that provider's API. These permissions are not used to inject scripts or read webpage content; the demo gateway also validates the browser's exact Origin. |
